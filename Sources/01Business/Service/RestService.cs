...
namespace NextRadio.Service
	...
	public static class RestService
		...
		//-----------------------------------------------------------------------------------------------------------------------
		private const string BaseUri  = "http://api.nextradiotv.com/01business-android/";
		private const string AppTitle = "01Business : emploi, formation et conseils IT pro";
		//-----------------------------------------------------------------------------------------------------------------------
		private static Regex InnerLinkNotify = new Regex ( @"^http\:\/\/pro\.01net\.com\/" +
		...
		static RestService ()
			...
			Colors[(long)SectionType.Products  ] = "2299AA";
			Colors[(long)SectionType.Security  ] = "BB0000";
			Colors[(long)SectionType.Service   ] = "559933";
			Colors[(long)SectionType.Technology] = "9955AA";
			Colors[(long)SectionType.Telecoms  ] = "886644";

			Colors[11410 + 10016] = "9955AA"; // SectionType.Technology
			Colors[10398        ] = "2299AA"; // SectionType.Products
			Colors[10005        ] = "BB0000"; // SectionType.Security
			Colors[11410 + 10023] = "559933"; // SectionType.Service
			Colors[11410 + 10019] = "886644"; // SectionType.Telecoms
			//-------------------------------------------------------------------------------------------------------------------
		...
		/// </summary>
		//-----------------------------------------------------------------------------------------------------------------------
		private static string SectionToColor ( long SectionID, long SubSectionID )
			{
			//-------------------------------------------------------------------------------------------------------------------
			return ( Colors.ContainsKey ( SectionID                ) ) ? Colors[SectionID               ] : 
			       ( Colors.ContainsKey ( SectionID + SubSectionID ) ) ? Colors[SectionID + SubSectionID] : "4B4B4B";;
			//-------------------------------------------------------------------------------------------------------------------
			}
		//***********************************************************************************************************************
		...
		//-----------------------------------------------------------------------------------------------------------------------
		private static void RegisterApp ( object AssociateObject, AppCacheFile CacheFile, EventHandler Callback, 
		                                                                                            RestEventHandler OnComplete )
			{
			//-------------------------------------------------------------------------------------------------------------------
			// Exécution
		...
		private static AppCacheFile ParseGetArticle ( Article Article, AppWebCache Cache, DownloadBitsState State )
			...
			Article.Color = SectionToColor ( Article.SectionID, Article.SubSectionID );
			//-------------------------------------------------------------------------------------------------------------------
			...
			//-------------------------------------------------------------------------------------------------------------------
			Content += string.Format ( SectHtml, Article.Color, Article.Section );
			...
			if ( string.IsNullOrEmpty ( Article.Author ) ) Article.Author = "01Business";
			...
			using ( var Bookmarks = AppBookmarks.GetForApplication () )
				...
				foreach ( var Item in Items.Keys )
					...
					if ( string.IsNullOrEmpty ( Item.Color ) )
						Item.Color = SectionToColor ( Item.SectionID, Item.SubSectionID );
		...
		private static void ParseGetLastArticles ( DownloadBitsState State          , 
			...
			using ( var Cache = AppWebCache.GetForApplication () )
				...
				if ( HeadlinesResult == RestRequestResult.Success && 
					...
					State.PostOperationComplete ( InsertSheets ( NewEntry.Content, true ) );
					//-----------------------------------------------------------------------------------------------------------
					}
				//---------------------------------------------------------------------------------------------------------------
		...
		private static void AsyncGetArticlesList ( RestEventHandler OnComplete, SectionType Type, AppCacheFile CacheFile )
			...
			//-------------------------------------------------------------------------------------------------------------------
				...
				// Initialisation
				//---------------------------------------------------------------------------------------------------------------
				string Path = "getLastArticles";

				switch ( Type )
					{
					//-----------------------------------------------------------------------------------------------------------
					case SectionType.Products   : Path += "?category=13"; break;
					case SectionType.Security   : Path += "?category=23"; break;
					case SectionType.Service    : Path += "?category=15"; break;
					case SectionType.Technology : Path += "?category=6";  break;
					case SectionType.Telecoms   : Path += "?category=10"; break;
					//-----------------------------------------------------------------------------------------------------------
					}
				//---------------------------------------------------------------------------------------------------------------
